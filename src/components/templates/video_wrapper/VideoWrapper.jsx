...
const VideoWrapper = ({ video_component, title, description, button_text, clickLogic, ...args }) => {
   const { color, record, username, idx, votes, disabled } = args;
      ...
      <Card className='m-1' style={{ border: idx % 2 == 1 ? '3.5px solid lightseagreen' : '3.5px solid red', height: '100%', borderRadius: '15.3px' }}>
         <Suspense fallback={<SuspenseFallback />}>
            ...
            <CardBody className='px-1'>
               ...
               { description && <CardText className='px-1' style={{paddingBottom: '5px', height: '100%'}}>{description}</CardText> }
               { (votes || votes === 0) && <CardText style={{paddingBottom: '5px'}}>VOTES: <span className='text-danger'>{votes}</span></CardText> }
            </CardBody>            
            {
               (button_text && clickLogic)
               &&
               <CardFooter className='w-100' style={{backgroundColor: 'black'}}>
                  <Button type="button" className= {`btn btn-${color ? color : 'danger'} btn-block w-100`} disabled={disabled} onClick={clickLogic || null}>{button_text}</Button>
               </CardFooter>
            }
         </Suspense>
      </Card>
