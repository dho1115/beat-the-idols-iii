      ...
      <Card className='m-1' style={{ border: idx % 2 == 1 ? '3.5px solid lightseagreen' : '3.5px solid red', height: '100%', borderRadius: '15.3px' }}>
         <Suspense fallback={<SuspenseFallback />}>
            <div className='w-100 p-0 cover-img-div'>
               <img src={coverImg} alt='cover pic for challenge wrapper' className='cover-img img-fluid'/>
            </div>
            ...
            <CardBody className='px-1'>
               <strong>Posted By: {username}</strong>
               {
                  ...
                  <CardText className='px-1' style={{paddingBottom: '5px', height: '100%'}}>
                     {description.length >= 31 ? <p>{description.substring(0, 30)}...</p> : <p>{description}</p>}
                  </CardText>
               }
            </CardBody>            
            {
